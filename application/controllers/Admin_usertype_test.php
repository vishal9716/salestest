...
class Admin_usertype_test extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('admin_usertype_model_test');
    }

    function index() {
        $this->load->view('admin_usertype_test');
    }

   function getData() {
		echo "controller test"; die;
		 $data['data'] = $this->admin_usertype_model_test->getData()->result_array();
      //echo "<pre/>";
		//print_r($data);
...
//        $this->load->view('admin_usertype', $data);
    ...
    function save() {
        ...
        if ($result->num_rows() > 0) {
            ...
            $this->admin_usertype_model_test->save($user_type);
