...
class Account_receivable extends CI_Controller {
	 ...
	 */
	 ...
	 public function __construct() {
		...
		// Load database
		$this->load->model('accountreceivable_database');
	...
	{		
		 ...
		 $result = $this->accountreceivable_database->receivablelist_info();
		...
		$this->load->view('account_receivable_list',$data);
	}
	
	public function add_account_receivable()
	{
		...
		if($this->input->post('segment'))
		{
			...
			$data = array(
				...
				$result_invoice_id = $this->accountreceivable_database->add_receivablelist($data);
		...
		{	
			...
			$this->load->view('add_account_receivable',$data);
		}
	...
	// Edit user	
	public function edit_account_receivable()
	{
		...
		if($this->input->post('segment'))
		{
			...
			$result_receivable = $this->accountreceivable_database->edit_receivablelist($data);
		...
		{	
				...
				$result_d = $this->accountreceivable_database->receivablelist_info_data($receivableid);			
			...
			$this->load->view('edit_account_receivable',$newData);	
		}
	...
	public function delete_receivable() {

		$receiveid = $_GET['rid'];
		$result_records = $this->accountreceivable_database->delete_receivable_data($receiveid);
		if($result_records ==1)
		{
			$result = $this->accountreceivable_database->receivablelist_info();
			$result_cust = $this->customer_database->customerlist_info();
