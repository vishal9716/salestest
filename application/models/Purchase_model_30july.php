...
$query="update purchase_request set supplier_name='".$supplier_name."', expense='".$expense."', pr_issue_date='".$issuing_date."',phone_person='".$phone_person."',action_taken_by='".$action_taken_by."', pr_recd_on='".$pr_reacd_on."',order_placed_by='".$order_placed_by."',status='Pending' where sr_no='".$pr_srno."'";
...
function display_purchase_request($id) {
		 ...
		 if ((!empty($id))){
            ...
            $sql = "select distinct sr_no, department_id,pr_issue_date,supplier_name,order_placed_by,phone_person,expense,action_taken_by,status from purchase_request;
";
        }
		//echo $sql; die;
        $result = $this->db->query($sql)->result_array();
...
for($i = 0; $i < count($vendor_selection); $i++)
 {
 $insertArr[] = "('".$pr_sr_no."','" . ($vendor_selection[$i]) . "','" . ($brand_selection[$i]) . "','" . ($bids[$i]) . "','" . ($negotiation_rounds[$i]) . "','" . ($sla_agreement[$i]) . "','" . ($agreement_late_delivery[$i]) . "','" . ($payment_agreement[$i]) . "','" . ($service_agreement[$i]) . "','" . ($amc_negotiation[$i]) . "','" . ($delivery_agreement[$i]) . "','" . ($insurance[$i]) . "','" . ($special_point[$i]) . "')";
 
 }
$sql = "INSERT INTO pr_audit_checklist(pr_srno,vendor_selection,brand_selection,bids,negotiation_rounds,sla_agreement,agreement_late_delivery,payment_agreement,service_agreement,amc_negotiation,delivery_agreement,insurance,special_point) VALUES " . implode(", ", $insertArr);
...
echo "Error in Adding Comarision sheet";
	...
	$sql = "insert into pr_internal_memo(pr_sr_no,date,to,from,subject,description) values('".$pr_sr_no."','".$date."','".$to."','".$from."','".$subject."','".$description."')";
	echo $sql; die;
	$result = $this->db->query($sql);
if($result){
echo "Internal Memo added successfully";
}
else{
echo "Error in Adding Internal Memo";
}	
	}
