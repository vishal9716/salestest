...
class Operations_model extends CI_Model {
   ...
   // List Activities
    ...
    if(($customer_name != "") || (!empty($customer_name))){
        $sql = "insert into customer(customer_name,customer_desp,customer_email,payment_term, customer_address) values('" . ($customer_name) . "','" . ($customer_descp) . "','" . ($email) . "','" . ($payment_terms) . "','" . ($address) . "')";
    //  echo $sql; die;
      $query = $this->db->query($sql);
