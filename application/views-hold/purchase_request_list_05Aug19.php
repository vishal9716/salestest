                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            <thead>
                                <tr>
									...
									<?php if($username=='parul'){ }else{?>  <th>Action</th> <?php } ?>
                                </tr>
                            </thead>								
                            <tbody>
...
<td id="pr_srno"><a prsno="<?php echo $list['sr_no']; ?>" class="prsno" href='#' data-toggle='modal' data-target='#prQuot'>
					...
					<td>
 <?php if($list['status']=='Approved'){$class ='success';} else if($list['status']=='Rejected'){$class ='danger';} else if($list['status']=='Pending'){$class ='warning';} ?>
	<!--<span class="label label-danger">Reject</span>-->
	<span title="<?php echo $list['pr_status']; ?> on : <?php echo date('Y-m-d', strtotime($list['pr_status_date'])); ?> &#013;<?php echo $list['status']; ?> by : <?php echo $list['status_by']; ?>" onclick="confirmation('<?php echo $list['sr_no']; ?>');" class="label label-<?php echo $class;?>"><?php echo $list['status']; ?></span>
                                            <!--<span class="label label-success">Approved</span>-->
                                        </td>
                                   <!-- <td><?php echo $list['phone_person']; ?></td>-->
	
										
										<?php if($username=='konain'){?> <td><a href="<?php echo base_url(); ?>index.php/purchase_request/edit_purchase_request?sr_no=<?php echo $list['sr_no']; ?>">Edit</a> | <a href="<?php echo base_url(); ?>index.php/purchase_request/internal_memo?sr_no=<?php echo $list['sr_no']; ?>">Memo</a> </td> <?php }

else if($username=='parul'){?>  <?php }
									else{ ?>
...
<script>
    $(document).ready(function () {
	 ...
	 function confirmation(pr_srno) {
        var r = confirm("Are you sure you want to Approve this Purchase Request");
		 //alert(r);
        
     $.ajax({
                    url: "<?php echo base_url(); ?>index.php/purchase_request/update_pr_status/"+r+"/"+pr_srno,
                    method: "POST",
                        success: function (data) {
                          alert(data);
